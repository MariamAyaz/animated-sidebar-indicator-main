...
export default function AddNewForRentProperty(props) {
    const { id } = useParams();
    ...
    useEffect(() => {
        axios.get('http://localhost:4000/ForRentProperties/edit-forRentProperty/' + id)
            .then((res) => {
                setPropertyTitle(res.data.PropertyTitle);
                setPropertyTagline(res.data.PropertyTagline);
                setAddress(res.data.Address);
                setCity(res.data.City);
                setPrice(res.data.Price);
                setDatePosted(res.data.DatePosted);
                setDescription(res.data.Description);
                setPropertyMapLocation(res.data.PropertyMapLocation);
                ...
                setLivingrooms(res.data.Livingrooms);
                setTypeOfProperty(res.data.TypeOfProperty);
                ...
                setAreaSqFt(res.data.AreaSqFt);
                setOwner(res.data.Owner);

                let count=0;
                res.data.Amenities.map((a => Amenities.push({id:JSON.stringify(count++), text:a })));
                
                count=0;
                res.data.Places.map((a => Places.push({id:JSON.stringify(count++), text:a })));

                count=0;
                res.data.PopularTags.map((a => PopularTags.push({id:JSON.stringify(count++), text:a })));
            })
            .catch((err) => alert(err));
    }, id);
    ...
    function submit(event) {
        ...
        axios.put('http://localhost:4000/ForRentProperties/edit-forRentProperty/' + id, property)
            .then(res => {
                alert("Property updated Successfully");
                navigate("/Admin/ForRentProperties", { replace: true });
            })
    ...
    return (
        <div className=' ps-5 pt-5 pb-5 me-5 mb-5 bg-white rounded'>
            <h3 className='text-primary text-center mt-3 mb-5'><strong>Edit Property</strong></h3>
